...
params = {
    "$filter": f"Language eq 'DE' and LegislativePeriodNumber ge {first_leg}",
    "$orderby": "ID",
}  # Council == 1 ist Nationalrat, == 2 ist Ständerat
...
with open(f"../Data/legislatures.json", "w") as file:
    json.dump(
        legislaturen_out.to_dict(orient="records"), file
    )  # default str makes everything that can not be serialized into a string (for example a date)
...
response_sel = response_sel[
    (response_sel["Language"] == "DE") | (response_sel["Language"] == "FR")
]
...
response_sel = pd.concat(
    [pd.DataFrame([{"de": "Alle Themen", "fr": "Tous les thèmes"}]), response_sel],
    ignore_index=True,
)
topics = response_sel
...
for council in [1, 2]:
    ...
    while repeat:
        ...
        params = {
            "$top": str(interval),
            "$skip": str(skip),
            "$filter": f"Language eq 'DE' and Council eq {council}",
        }  # Council == 1 ist Nationalrat, == 2 ist Ständerat
        response = requests.get(api_url, headers=headers, params=params)
        response = response.json()["d"]
        response_pd = pd.json_normalize(response)
        if abgeordnete_hist is None:
            abgeordnete_hist = response_pd.copy(deep=True)
        else:
            abgeordnete_hist = pd.concat(
                [abgeordnete_hist, response_pd], ignore_index=True
            )
    ...
    abgeordnete_hist_sel = abgeordnete_hist[
        [
            "PersonNumber",
            "FirstName",
            "LastName",
    ...
    abgeordnete_hist_sel["DateJoining"] = abgeordnete_hist_sel["DateJoining"].apply(
        parse_date
    )
    abgeordnete_hist_sel["DateLeaving"] = abgeordnete_hist_sel["DateLeaving"].apply(
        parse_date
    )
    abgeordnete_hist_sel["DateElection"] = abgeordnete_hist_sel["DateElection"].apply(
        parse_date
    )
    abgeordnete_hist_sel = abgeordnete_hist_sel.sort_values(
        ["PersonNumber", "DateJoining"]
    )
    abgeordnete_hist_sel = abgeordnete_hist_sel.drop_duplicates(
        ["PersonNumber", "DateLeaving", "DateJoining"], keep="last"
    )  # Einige Einträge können doppelt vorkommen durch änderungen in nicht relevanten spalten (e.g. Mitglied wurde zu Präsident oder Umzug etc.), Einige sind doppelt in relevanten Spalten (e.g. Parteiwechsel) allerdings gibt es keine Informationen über die Reihenfolge wechsel von wo nach wo..
    ...
    while repeat:
        ...
        response = response.json()["d"]
        response_pd = pd.json_normalize(response)
        if abgeordnete is None:
            abgeordnete = response_pd.copy(deep=True)
        else:
            abgeordnete = pd.concat([abgeordnete, response_pd], ignore_index=True)
        if len(response_pd) < interval:
            repeat = False
        else:
            skip += interval
    ...
    abgeordnete = abgeordnete[
        abgeordnete.apply(
            lambda x: x["PersonNumber"]
            in abgeordnete_hist_sel["PersonNumber"].unique(),
            axis=1,
        )
    ]  # select only the ones that at one time sat in the selected council
    abgeordnete_sel = abgeordnete[
        [
            "PersonNumber",
            "FirstName",
            "LastName",
    ...
    abgeordnete_sel["GenderAsString"] = abgeordnete_sel["GenderAsString"].apply(
        lambda x: "weiblich" if (x == "f") else "männlich"
    )
    ...
    def agg_parties(party, leg):
        ...
        if party == "CSI":
            return "CSP"
        elif party == "CSPO" and leg < 51:
            return "CVP"
        elif party == "CSPO" and leg >= 51:
            return "M-E"
        elif party == "CVPO" and leg < 51:
            return "CVP"
        elif party == "CVPO" and leg >= 51:
            return "M-E"
        elif party == "LDP" and leg >= 48:
            return "FDP-Liberale"
        elif party == "GB":
            return "GRÜNE"
        elif party == "BastA":
            return "GRÜNE"
        elif party == "Al" and leg >= 48:
            return "GRÜNE"
        ...
        # This illustrates the coninuity and avoids confusion
            ...
            return "MCG"
        elif party == "CVP" and leg >= 51:
            return "M-E"
        elif party == "BDP" and leg >= 51:
            return "M-E"
        elif party == "FDP" and leg >= 48:
            return "FDP-Liberale"
        else:
            return party
    ...
    abgeordnete_sel["Legislaturen"] = (
        None  # needs to be initialized as datatype object to be able to assign lists
    )
    abgeordnete_sel["DienstjahreTot"] = None
    abgeordnete_sel["DienstjahreLeg"] = None
    abgeordnete_sel["PartyAbbreviation"] = abgeordnete_sel["PartyAbbreviation"].astype(
        object
    )
    abgeordnete_sel["ParlGroupAbbreviation"] = abgeordnete_sel[
        "ParlGroupAbbreviation"
    ].astype(object)
    ...
    for p in person_numbers:
        ...
        for index, leg in individual.iterrows():
            ...
            legs_entry = legislaturen[
                (
                    (
                        (leg["DateJoining"] >= legislaturen["StartDate"])
                        & (leg["DateJoining"] < legislaturen["EndDate"])
                    )
                    | (
                        (
                            leg["DateLeaving"]
                            > (legislaturen["StartDate"]) + timedelta(days=1)
                        )
                        & (leg["DateLeaving"] <= legislaturen["EndDate"])
                    )
                )  # Subtract one day from Date Leaving to counter some bad data where DateLeaving (add to Start Date) is larger than StartDate of next Leg (see 249)
                | (
                    (leg["DateJoining"] <= legislaturen["StartDate"])
                    & (leg["DateLeaving"] >= legislaturen["EndDate"])
                )
            ...
            if leg["DateJoining"] <= legislaturen.loc[first_leg]["StartDate"]:
                if leg["DateLeaving"] <= legislaturen.loc[first_leg]["StartDate"]:
                    before = (
                        leg["DateLeaving"] - leg["DateJoining"]
                    ).days / 365.25  # transform from days to years. Not fully acurate but close enough.
                else:  # includes also time in first_leg. Leg or later
                    before = (
                        legislaturen.loc[first_leg]["StartDate"] - leg["DateJoining"]
                    ).days / 365.25
            tot_service_count += before
            # check time in each Legislatur that the entry covers
            for l in legs_entry:
                # Start date
                if leg["DateJoining"] <= legislaturen.loc[l]["StartDate"]:
                    startdate = legislaturen.loc[l][
                        "StartDate"
                    ]  # Sometimes entries span larger timespans including years befor Leg 45 eg. 39
                else:
                    startdate = leg["DateJoining"]

                # End date
                if leg["DateLeaving"] is None:
                    enddate = date.today()
                elif leg["DateLeaving"] >= legislaturen.loc[l]["EndDate"]:
                    enddate = legislaturen.loc[l]["EndDate"]
                else:
                    enddate = leg["DateLeaving"]
                ...
                if leg["DateLeaving"] is not None:
                    ...
                    leg_party[str(l)] = agg_parties(
                        abgeordnete_sel.loc[leg["PersonNumber"]]["PartyAbbreviation"], l
                    )
                    leg_parlg[str(l)] = abgeordnete_sel.loc[leg["PersonNumber"]][
                        "ParlGroupAbbreviation"
                    ]
                    leg_canton[str(l)] = abgeordnete_sel.loc[leg["PersonNumber"]][
                        "CantonName"
                    ]
        legs = np.unique(legs)
        abgeordnete_sel.at[p, "Legislaturen"] = [
            l.item() for l in legs
        ]  # need to use at instead of loc to assign lists, use .item() to convert numpy int to int
    ...
    for request_leg in legislaturen.ID:
        ...
        if (
            path.isfile(f"../Data/netzwerk_{request_leg}_{council}.json")
            and request_leg != legislaturen.ID.iloc[-1]
        ):  # update the last leg
            print(f"File for leg {request_leg} already exists", flush=True)
            continue
        else:
            print("Working on Legislature:", request_leg, flush=True)
        abgeordnete_sel_leg = abgeordnete_sel[
            abgeordnete_sel["Legislaturen"].apply(lambda x: request_leg in x)
        ]
        abgeordnete_sel_leg["DienstjahreTot"] = abgeordnete_sel_leg[
            "DienstjahreTot"
        ].apply(lambda x: x[str(request_leg)])
        abgeordnete_sel_leg["DienstjahreLeg"] = abgeordnete_sel_leg[
            "DienstjahreLeg"
        ].apply(lambda x: x[str(request_leg)])
        abgeordnete_sel_leg["PartyAbbreviation"] = abgeordnete_sel_leg[
            "PartyAbbreviation"
        ].apply(lambda x: x[str(request_leg)])
        abgeordnete_sel_leg["ParlGroupAbbreviation"] = abgeordnete_sel_leg[
            "ParlGroupAbbreviation"
        ].apply(lambda x: x[str(request_leg)])
        abgeordnete_sel_leg["CantonName"] = abgeordnete_sel_leg["CantonName"].apply(
            lambda x: x[str(request_leg)]
        )
        abgeordnete_sel_leg = abgeordnete_sel_leg.rename(
            {
                "PartyAbbreviation": "Partei",
                "ParlGroupAbbreviation": "Fraktion",
                "GenderAsString": "Geschlecht",
                "PersonNumber": "id",
            },
            axis=1,
        )
        ...
        if len(abgeordnete_sel["PersonNumber"].unique()) != len(
            abgeordnete_sel["PersonNumber"]
        ):
        ...
        test["debug"] = test.apply(
            lambda x: len(x["Legislaturen"]) != len(set(x["Legislaturen"])), axis=1
        )
        ...
        for halve in [1, 2]:
            ...
            if halve == 1:
                ...
                end_date = legislaturen.loc[request_leg]["StartDate"] + relativedelta(
                    years=2
                )
            elif halve == 2:
                start_date = legislaturen.loc[request_leg]["StartDate"] + relativedelta(
                    years=2
                )
            ...
            while repeat:
                ...
                try:  # check if successfull
                    response = response.json()["d"][
                        "results"
                    ]  # varialbe results only exists if expand is used or top is not used
                except:
                    time.sleep(2)
                    retries += 1
                    print(
                        f"Request failed at skipt {skip}. Retries: {retries}",
                        flush=True,
                    )
                    if retries == 5:
                        interval -= 100
                        ...
                        continue  # try the same request again (max 10 times)
                retries = 0  # reset retries for the next request
                response_pd = pd.json_normalize(response)
                if business is None:
                    business = response_pd.copy(deep=True)
                else:
                    business = pd.concat([business, response_pd], ignore_index=True)
                if len(response_pd) < interval:
                    repeat = False
                else:
                    skip += interval  # Limit sind 1000 pro Abfrage aber führt oft zu timeout
            if len(business) > 0:  # only if there are results
        ...
        def normalize_roles(df):
            ...
            if list(pd.json_normalize(df).columns) == []:
                return None
            else:
                out = out[["MemberCouncilNumber", "RoleName"]]
                out = out[
                    (out["RoleName"] == "Mitunterzeichner(-in)")
                    | (out["RoleName"] == "Urheber(-in)")
                ]  # Filter out something like Bekämpfer(-in)
        ...
        def merge_types(type):
            if type == "Dringliche Anfrage":
                return "Anfrage"
            elif type == "Dringliche Interpellation":
                return "Interpellation"
            else:
                return type

        business_sel = business_tot[
            business_tot["SubmittedBy"].str.contains(
                "nationalrat|kommission|fraktion", case=False
            )
            == False
        ]
        ...
        business_sel = business_sel[
            [
                ...
                "BusinessTypeName",
                "TagNames",
                "BusinessRoles.results",
            ]
        ]
        # business_sel['SubmissionDate'] = business_sel['SubmissionDate'].apply(parse_date)
        business_sel["BusinessRoles.results"] = business_sel[
            "BusinessRoles.results"
        ].apply(normalize_roles)
        business_sel = business_sel.rename(
            {"BusinessRoles.results": "BusinessRoles"}, axis=1
        )
        business_sel = business_sel.set_index("BusinessShortNumber", drop=False)
        business_sel["TagNames"] = business_sel["TagNames"].apply(
            split_tags
        )  # Transform Tags to list
        business_sel = business_sel.rename(
            {
                "BusinessShortNumber": "id",
                "BusinessTypeName": "Type",
                "TagNames": "Thema",
            },
            axis=1,
        )
        business_sel["Type"] = business_sel["Type"].apply(
            merge_types
        )  # Merge Urgent with normal businesses
        ...
        for b, bus in business_sel.iterrows():
            if not bus["BusinessRoles"]["PersonNumber"].all():
                print(b, flush=True)
        ...
        for b, bus in business_sel.iterrows():
            if bus["Type"] == "Dringliche Anfrage":
                print(b, flush=True)
        ...
        for b, bus in business_sel.iterrows():
            if "Bekämpfer(-in)" in bus["BusinessRoles"]["RoleName"].values:
                print(b, flush=True)

        for b, bus in business_sel.iterrows():
            if len(bus["BusinessRoles"]) == 0:
                print(b, flush=True)
        ...
        for b, bus in business_sel.iterrows():
            for a, abg in bus["BusinessRoles"].iterrows():
                if (
                    abg["RoleName"] == "Mitunterzeichner(-in)"
                    or abg["RoleName"] == "Urheber(-in)"
                ):
                    if a in collab.keys():
                        collab[a].append(b)
                    else:
        ...
        for a, abg in abgeordnete_sel_leg.iterrows():
            if a in collab.keys():
                abgeordnete_sel_leg.at[a, "Collab"] = business_sel.loc[collab[a]].drop(
                    "BusinessRoles", axis=1
                )
            if a in lead.keys():
                abgeordnete_sel_leg.at[a, "Lead"] = business_sel.loc[lead[a]].drop(
                    "BusinessRoles", axis=1
                )
        ...
        # Speeeeed
        links = {}
        business_sel_iter = business_sel[
            business_sel["BusinessRoles"].apply(lambda x: len(x) > 1)
        ]
        ...
        for b, bus in business_sel_iter.iterrows():
            # for b, bus in business_sel.iterrows():
            comb = list(
                itertools.combinations(bus["BusinessRoles"]["PersonNumber"].values, 2)
            )
            ...
            for l in comb:
                ...
                if str(l) not in links.keys():
                    sameParty = (
                        abgeordnete_sel_leg.loc[l[0]].Partei
                        == abgeordnete_sel_leg.loc[l[1]].Partei
                    )
                    links[str(l)] = {
                        "source": int(l[0]),
                        "target": int(l[1]),
